...
// Seed prose for novel-type chapters — the reader renders these as real paragraphs, not placeholder page images.
const NOVEL_PARAGRAPHS = [
  "لم يكن يعلم حين خرج من القرية تلك الليلة أن الطريق الذي اختاره سيغيّر كل ما يعرفه عن نفسه. الرياح كانت باردة، والقمر معلّق خلف غيوم رقيقة، وكل خطوة كانت تبعده أكثر عن الحياة التي عاشها طفلًا.",
  "توقّف عند حافة الغابة، ينظر إلى الظلال المتراقصة بين الأشجار. شيء في داخله يهتف بالعودة، وشيء آخر أقوى يدفعه للأمام. اختار أن يستمع للصوت الثاني.",
  "\"لن أطلب منك أن تفهم،\" قالت بصوت هادئ لا يخلو من الحزن، \"لكنني أطلب منك أن تثق بي، حتى لو بدت كل الطرق مغلقة أمامنا.\"",
  "مرّت الأيام ثقيلة كالحجر. كل صباح كان يبدأ بنفس السؤال: هل يستحق الأمر كل هذا العناء؟ وكل مساء كان يجد إجابة جديدة، مختلفة عن سابقتها، لكنها تقوده دائمًا إلى نفس القرار: الاستمرار.",
  "عندما واجه الأمر أخيرًا، لم يكن كما تخيّله. لم يكن هناك بريق سيف أو صرخة معركة، بل صمت ثقيل، وعينان تنظران إليه من الداخل كأنهما تعرفانه منذ زمن بعيد.",
  "الذكريات ليست شيئًا يمكن دفنه بسهولة. كانت تعود إليه في أضعف اللحظات، تتسلل بين أنفاسه وتذكّره بمن كان، وبمن يمكن أن يعود إليه إن لم يكن حذرًا.",
  "قال المعلم العجوز، وهو يشير إلى الأفق البعيد: \"القوة الحقيقية لا تُقاس بما تهزمه، بل بما تختار أن ترحمه رغم قدرتك على تحطيمه.\"",
  "شعر بثقل الاختيار يجلس على كتفيه كجبل. لم يعد الأمر يتعلق به وحده؛ كل من يحبهم أصبحوا جزءًا من الميزان الذي يجب أن يوازنه بحكمة لم يكتسبها بعد.",
  "الميدان كان فارغًا إلا من صدى خطواته وضوء الفجر الزاحف على الحجارة القديمة. هنا، بين هذه الجدران المتهالكة، كانت القصة كلها قد بدأت — وهنا كان مقدَّرًا لها أن تجد إجابتها.",
  "ابتسم بخفة، ابتسامة من عرف أن المعركة القادمة ليست الأخيرة، لكنها الأهم. \"إذن،\" قال لنفسه بصوت لا يسمعه غيره، \"لنر ما الذي سنصبح عليه بعد الليلة.\"",
];

function buildNovelContent(rng: Rand): string {
  const count = rng.int(9, 16);
  return Array.from({ length: count }).map(() => rng.pick(NOVEL_PARAGRAPHS)).join("\n\n");
}
...
function build(): MockDatabase {
  ...
  const teams: Team[] = TEAM_NAMES.slice(0, TEAM_COUNT).map((name, i) => {
    ...
    return {
      ...
      goals: `نسعى ليصبح فريق ${name} من أفضل فرق الترجمة عربيًا من ناحية الجودة والانتظام بالنشر.`,
  ...
  const series: Series[] = Array.from({ length: SERIES_COUNT }).map((_, i) => {
    ...
    return {
      ...
      synopsis:
        "في عالم يحكمه القوة والدهاء، يخوض بطلنا رحلة صعود مليئة بالتحديات والمؤامرات، باحثًا عن الانتقام والمجد في آنٍ واحد. هل سيتمكن من كسر قيوده والوصول إلى القمة؟",
      type: rng.pick(types),
      status,
  ...
  for (const s of series) {
    ...
    for (let c = 0; c < count; c++) {
      ...
      chapters.push({
        ...
        number,
        title: `الفصل ${number}`,
        ...(s.type === "novel" ? { pages: 0, content: buildNovelContent(rng) } : { pages: rng.int(18, 45) }),
  ...
  const ACTIVITY_ACTIONS = [
    "أضاف عضوًا جديدًا للفريق",
    "أزال عضوًا من الفريق",
    "غيّر دور عضو",
    "عدّل صلاحيات دور",
    "نشر فصلًا جديدًا",
    "أنشأ مشروعًا جديدًا",
  ...
  const recruitmentApplications: RecruitmentApplication[] = recruitmentPositions.flatMap((pos, idx) => {
    ...
    return rng.pickMany(nonMemberUsers, applicantCount).map((applicant, i) => ({
      ...
      availability: rng.pick(["جزئي (أقل من 10 ساعات أسبوعيًا)", "متوسط (10-20 ساعة أسبوعيًا)", "كامل (أكثر من 20 ساعة أسبوعيًا)"]),
  ...
  const teamCreationRequests: TeamCreationRequest[] = PENDING_TEAM_NAMES.map((name, i) => {
    ...
    return {
      ...
      goals: "نشر فصل واحد أسبوعيًا على الأقل مع الحفاظ على جودة الترجمة والتنسيق.",
      discordUrl: "https://discord.gg/example",
      requiredPositions: rng.pickMany(RECRUIT_ROLES, rng.int(2, 4)),
      category: rng.pick(teamCategories),
      expectedMembers: rng.int(5, 20),
      previousExperience: rng.pick([
        "أعضاء الفريق عملوا سابقًا في فرق ترجمة معروفة",
