...
**Three ways to give a chapter its pages**, all ending in the same pipeline (`ChaptersService.storePage`: check the picture, re-encode it as WebP which drops EXIF, checksum, store under a random key, link as page *n*), so every page is treated identically whatever its origin:
