...
## §28 — The site requires an account

By default no page is open to a visitor without a session: `src/middleware.ts` sends them to `/login?next=<where they were going>` (the login page returns them there afterwards; only a path on this site is accepted), and data routes under `/api` answer 401 `login_required`. Open to everyone: `/login`, `/register`, `/forgot-password`, `/terms`, `/privacy` (the sign-up form links to the rules), the auth API (`/api/auth/*`, which the matcher excludes) and static files (logo, fonts, robots.txt, sitemap.xml). `robots.txt` disallows everything but those pages and the sitemap lists only them, since crawlers would just be redirected. The switch is `SITE_REQUIRES_LOGIN` (default on; `false` in the frontend's environment opens the site again with no other change — `src/lib/access-policy.ts`).

Limits worth knowing. The gate at the door looks at the session cookies, and a forged cookie would get past it: it is a product rule, not the security boundary. What actually protects things is unchanged and server-side — page images are issued only against a valid access token (§22), personal data needs one, and the backend re-checks roles from the database. The catalogue endpoints (`@Public()`, §27) remain readable by anyone who calls them directly. If the refresh call fails (an expired or revoked session) the cookies are cleared and the visitor is sent to sign in; if the backend is unreachable the door fails open for that request rather than locking everyone out. Consequences of the rule: search engines index only the public pages, and a link to a series shared on Discord or elsewhere lands on the login page for someone without an account.
...
1. **Consent, separately.** The popunder and social bar load only for a visitor who agreed to advertising: its own cookie, `lunex-consent-ads` (`yes`/`no`, 6 months), written by the cookie banner ("accept all" = preferences + ads; "essential only" = neither). It is a separate cookie on purpose: an earlier "accept all" only ever covered preferences, so those visitors are asked again. Withdrawing consent (footer → cookie settings → essential only) reloads the page without the scripts. The privacy policy names the network and what it can collect (`/privacy`, "الإعلانات").
...
3. **Scripts are injected by the loader, not written into the layout**, so points 1 and 2 hold: the popunder goes into `<head>` and the social bar at the end of `<body>` as the network asks, each once (`data-lunex-ad`).
4. **The smartlink is a link, not a script.** It is shown labelled "رابط إعلاني" (footer, and the "watch an ad" box of a locked chapter) with `rel="sponsored nofollow noopener noreferrer"` and only leaves the site when clicked, so it needs no consent.
5. **Kill switch.** `NEXT_PUBLIC_ADS_ENABLED=false` (frontend build environment) removes all of it — scripts, links and the ad wording — without a code change.
