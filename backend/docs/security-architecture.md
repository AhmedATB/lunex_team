...
**Still kept in the visitor's own browser** (`store/team-management.ts`, `store/profile.ts`, preferences), i.e. **not shared and not persisted on the site**: the team dashboard's departments, custom roles and permissions, member promotion/demotion and removal buttons, collaboration requests between teams, the team activity log, team-creation requests (`/admin/team-requests` and `/teams/create`), and novel chapters created from the admin page. They are candidates for the same treatment, in that order of usefulness; until then anything decided there is visible only to whoever did it. Everything else listed in §34–§38 (teams' info, status and leader, members added or removed from *Admin › Teams*, works and their covers, moving works, pinning, news, chapters, messages, notifications, comments, recruitment, the user list) is on the server.
