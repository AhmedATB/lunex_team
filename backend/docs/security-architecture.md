...
**Enforcement status — read before assuming.** Bans are enforced server-side at sign-in, refresh and OAuth. Timeouts are enforced *in the UI only* for now: comments and messages are still device-local (§4), so there is nothing server-side to block. `GET /v1/auth/me` returns `mutedUntil` and the composers disable themselves; a determined user could bypass that until comments move to the server, at which point `ModerationService`'s `isMuted()` is the check to call. Sanction records are deleted with the account (cascade) and included in the data export.
