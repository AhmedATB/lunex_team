...
**Limits worth knowing.** A patient script can still open chapters, wait 15 seconds and finish them, up to the 300/day ceiling; experience has no monetary value here, so the ceiling is the whole defence. Novels are not real backend chapters yet and earn nothing. The coin store and locked-chapter unlocking are still client-side (`store/rewards.ts`) and unchanged. Concretely, the lock a reader sees is drawn by the browser (the newest three chapters of a series, with coins, tickets and "ad" counters kept in `localStorage`, where anyone can edit them); the server does not lock imported chapters at all (`manualLock = false`, because the old site had no locked chapters), and `POST /v1/chapters/:id/unlock` grants an unlock to any signed-in account without checking anything, so even a chapter the server did lock could be opened with one request. The lock is therefore cosmetic today; making it real needs server-side ad credits (or payments) behind that endpoint.
