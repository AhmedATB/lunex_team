...
**Reads.** `GET /v1/catalog/bootstrap` returns everything a page render needs in the frontend's own shapes (`Series`, `Team`, `Chapter`, `User`-like people, tags as genres) — series with derived chapter counts / latest chapter / favorites, the 60 newest chapters, news, teams ranked by output, and top readers. It is deliberately not paginated yet (a few hundred series is a small payload and the explore page filters client-side) and is memoised for 15 s in `CatalogService`; every catalogue write calls `invalidate()`. Catalogue reads are `@SkipThrottle()` because the per-IP limiter sees the whole site as one IP (§24). `topReaders` lists accounts with a public profile that have earned experience (§ progression: the reading *history* setting does not matter — only the counters show).
...
The site no longer requires an account to look around. A visitor with none is a **follower**: `src/middleware.ts` (policy in `src/lib/access-policy.ts`) lets them open the browsing pages — home, `/series`, a series' own page, `/search`, `/news`, `/teams` and a team's page, plus the sign-in pages and the rules (`/terms`, `/privacy`) — **and the chapter reader** (`/series/<slug>/<chapter>`), and, for `GET` only, the data those pages load: the catalogue (`/api/catalog/*` except the owner's `import/*` and `admin/*`), the comment threads (`/api/comments`, not its moderators' `admin` list) and a chapter's list and details (`/api/chapters`), plus the one write of asking for a page of a chapter to read (`POST /api/chapters/<id>/pages/<n>/token`). Everything else sends them to `/login?next=<where they were going>` (pages) or answers 401 `login_required` (data routes): commenting, rating, the library, messages, the store, notifications, profiles, team dashboards, `/teams/create` and the admin area.

**Chapters: open ones are readable by anyone, locked ones are not.** The lock (§33) is decided where the pages are issued: `POST /v1/chapters/:id/pages/:n/token` is `@Public()`, and for a caller with no account the wallet answers `canRead = !locked` (`WalletService.access(null, …)` — no account means no unlock and no staff role to consult). A chapter that is not locked gets its page token (recorded in the image log as reader `guest`, with the device and address beside it, and still subject to the per-device scraping limit); a locked one is refused with 403 `chapter_locked`, and the reader shows a screen asking the visitor to sign in to open it with reading credits or coins. An unpublished chapter is never issued to anyone (the wallet finds only published chapters). Signing in from a locked page returns the visitor to it (only a path on this site is accepted as `next`). The old `SITE_REQUIRES_LOGIN` switch is gone.
...
**Who sees what.** The owner always sees all of it. Level, experience, streak, chapter count and achievements are **counters**, shown on any profile that is visible to the viewer whatever `historyVisibility` says; that setting hides only the list of *which works* were read (and the last chapter of each). The home page's *top readers* — ranked by experience — lists every account with a **public profile** (`profileVisibility = public`) that has earned some, and shows only those counters; an account that hides its whole profile is left out, which is the way to opt out. (Before, both followed the history setting, which is private by default, so the ranking was empty for everyone who had not opted in.) The account export includes the progression fields; deleting the account removes them with the row.
...
- `POST /v1/catalog/series/transfer { seriesIds, teamId }` moves works to a team (empty `teamId` = off every team). Same rule as changing one series' team — global editors only (owner, super administrator, editor). It is one transaction: the series and their **chapters** (`chapters.teamId` records who published each chapter, and the team dashboards and "last active" read it — it is null for a work with no team, §39) move together; an unknown series or team changes nothing (404). Audited as `catalog.series_transferred`.
...
**Not built yet.** Message editing, attachments, and notifications for messages (the unread number is the only signal). Deleting one's own messages and blocking arrived in §44.

## §37 — Uploading a chapter: images, a ZIP, or a Google Drive folder

**Three ways to give a chapter its pages**, all ending in the same pipeline (`ChaptersService.storePages`: check the picture, cut or shrink it if it is too big (§39), re-encode it as WebP which drops EXIF, checksum, store under a random key, link as the next page(s)), so every page is treated identically whatever its origin:
- **Images from the device** and **a ZIP**: done in the browser. A ZIP is opened *there* (`lib/zip-reader.ts` reads the archive's index and inflates each picture on demand, without uploading the archive), the pictures are put in reading order by natural sort of their names (2 before 10; folders, `__MACOSX`, hidden files and non-images are left out) and sent one page at a time (three at once) with `POST /v1/chapters/:id/pages`. Nothing about the archive reaches the server, which removes the memory and zip-bomb concerns of unpacking uploads there.
- **Google Drive**: needs the server (it holds the key). `POST /v1/chapters/:id/import/drive { link }` (publishers only, 10 a minute) reads the folder with a Google **service account** (`modules/chapters/drive`, `GoogleDriveClient`: a signed JWT for `drive.readonly`, then plain HTTPS to the Drive API — no client library), lists its images, and answers at once with how many; the pages are fetched and stored in the background (three tries per download) and `GET /v1/chapters/:id/import/status` reports `{ state, total, done, error }`. The report lives in memory, one job per chapter (a restart forgets the report, never the pages). At most 300 images; imported pages go after any the chapter already has.
- **Public or private folders.** A folder shared as "anyone with the link" is readable by the service account as by any Google account; a private one has to be shared with the account's address, which the upload dialog shows (from `GET /v1/chapters/import/drive/info`). Google answers *not found* (or an empty list) for a folder the account cannot see, so the folder is checked first and reported as private rather than as empty.
- **The key** is an owner-held secret: set `GOOGLE_DRIVE_CREDENTIALS_JSON` on the backend service (the key file's content, or its base64). It is never sent to the browser or written to the repository; without it the Drive tab says the feature is not set up.

**Failure leaves nothing behind.** The dialog creates the chapter, fills it, then publishes it (or keeps a draft); if a page or the import fails, the half-made chapter is deleted. Publishing (as always) notifies the members who have the series in their library (§35).

**Not covered.** Sub-folders of a Drive folder, Dropbox/Mega links, and novel chapters (text), which are still created from the browser's own store; making novels real is separate work.

## §38 — Team recruitment on the server, and what is still kept per browser

**Recruitment is real** (`modules/recruitment`, tables `recruitment_positions` and `recruitment_applications`, migration `20260930180000_add_recruitment`). Before, positions and applications lived in each visitor's browser: a member's application never reached the leader, and a leader's "open position" showed only on their own screen. Now:
- A team's **leaders** (its leader, members with a leader role, and the site's owner, super administrators and team managers) open, close and delete positions; an open position is public (`GET /v1/recruitment/teams/:id/positions`, `?all=true` adds closed ones for leaders). A team shows as **recruiting exactly while it has an open position** — the flag on the team is recomputed after every change and the catalogue cache dropped, so the badge on the team's page, the teams list and each of its series follows.
- Any signed-in member applies (`POST …/applications`): to an open position (the application takes the position's role) or generally to a team that is recruiting; not if already on the team, not with an undecided application to the same team, at most five a day, not to a team that is suspended or archived. The team's leaders get a notification with a link to the dashboard; a team with nobody in charge (imported from the old site) sends it to the site's owner so none is lost.
- Leaders **review** (`PATCH /applications/:id`): accept (the applicant joins the team in the chosen role, default the one applied for), interview, waiting list or refuse — each with an optional note and each notifying the applicant (§35); a decided application cannot be decided again; an applicant can withdraw one that is undecided. Decisions are audited (`recruitment.accepted` …).
- Someone added straight to a team by an administrator (§34) is told too.

**Still kept in the visitor's own browser** (`store/team-management.ts`, `store/profile.ts`, preferences), i.e. **not shared and not persisted on the site**: the team dashboard's departments, custom roles and permissions, member promotion/demotion and removal buttons, collaboration requests between teams, the team activity log, and novel chapters created from the admin page (team-creation requests moved to the server, §42). They are candidates for the same treatment, in that order of usefulness; until then anything decided there is visible only to whoever did it. Everything else listed in §34–§38 (teams' info, status and leader, members added or removed from *Admin › Teams*, works and their covers, moving works, pinning, news, chapters, messages, notifications, comments, recruitment, the user list) is on the server.

## §39 — Long pictures are cut into pages, wide ones shrunk, and a work with no team can publish

**The failure this fixes:** importing a chapter from Google Drive stopped after the first page with "Page image dimensions exceed the allowed maximum" — a webtoon strip is often 10 000–30 000 pixels tall, and the upload refused any side over 6 000 px.

**What happens now** (`chapters/page-image.util.ts`, `preparePage`), for the admin upload, the ZIP and the Drive import (the bot's route, `POST /v1/integrations/bot/chapters/:id/pages`, keeps the old one-picture-one-page rule, because the bot numbers its own pages):
- a picture wider than **2 400 px** is shrunk to that width, keeping its proportions — wider canvases fail on phones (iOS caps a canvas near 16 M pixels) and a page is shown at a fraction of that width anyway;
- a picture taller than **4 500 px** is cut into `ceil(height / 4000)` pieces of about even height, each stored as its own WebP page, in order. Each cut is moved (at most 12 % of a piece's height either side of the even line) to the **emptiest stretch of rows**: rows are sampled from a thin greyscale copy, a row whose brightest and darkest sampled points differ by ≤ 12 counts as empty, and the cut goes to the middle of the longest empty run — so a speech bubble or a face is not sliced. With no empty row in reach it cuts at the calmest row. Pieces are stacked without a gap in the reader, so a cut in an empty area is invisible;
- a picture over 150 million pixels, or bytes that are not a picture, is still refused (`image_too_large`, `invalid_image`). EXIF orientation is applied before anything else (before, a rotated phone photo lost its orientation together with its EXIF).
- `storePages` answers how many pages a picture became. The Drive import numbers the next picture after them; the upload dialog now sends its pictures **one after the other** (it used to send three at once) for the same reason — the reader asks for pages `1..N` with no gaps, so page numbers must follow the pieces. Numbers are checked for a clash before the first piece is stored.

**A work with no team** (`chapters.teamId` is now nullable, migration `20260930200000_chapter_team_optional`; `POST /v1/chapters` takes `teamId` optional, an empty one means none): the upload dialog no longer refuses a work that belongs to no team — its chapters are published by the site's staff on their own. Removing a work from every team (`transfer` with an empty `teamId`) now clears its chapters' team too, so the two never disagree. The DTO still shows a chapter without a team as `teamId: ""`, as it does for a series.

**A chapter published from the panel did not show in "latest chapters"** (fixed): `PATCH /v1/chapters/:id` set `isPublished` but left `publishedAt` empty, and the catalogue's latest list (`recentChapters`), each work's `latestChapterAt` and each team's last activity all order by `publishedAt` with empty values last — so a chapter published here sat after every imported one, out of the top 60. Now going live stamps `publishedAt` (taking a chapter down clears it; a lock change or a repeated publish leaves it), and publishing, unpublishing and deleting a chapter drop the catalogue's 15-second cache so the home page and the work's page change at once. Migration `20260930210000_backfill_chapter_published_at` dates the chapters already live with no time from their creation. A work still has to be **approved** to appear in the list.

## §40 — The privacy policy and terms brought up to date (26 September 2026)

Both pages had fallen behind the features. **Privacy** now says: visitors read open chapters without an account and their page requests are logged like anyone's (IP, hashed device, event; 180 days); the level and chapter counter are counters shown on a visible profile and in the home page's top readers (public profile), separate from the reading *history*; **messages** are stored on the server, visible to the conversation's members only, not deletable yet, and stay for the others (without the sender's name) when an account is deleted; **team applications** are stored and visible to that team's leaders and site administrators; Cloudflare is named among the parties traffic passes through; the ad network may open a new window; what stays only in the browser is now the reader/appearance settings, the team dashboard's local data and, for a signed-out visitor, favorites and progress (ratings and messages are no longer there). **Terms** gained reading without an account, private-message conduct, publishing for a team or the site (uploader's responsibility for the rights; staff may move, hide, delete and pin works) and team applications.

The policy promised that "download my data" holds everything stored about the account, but the export left out ratings and opened chapters (collected, never returned) and had no wallet, messages, chats or applications. `GET /v1/users/me/export` now returns `ratings`, `openedChapters`, `wallet` (coins, reading credits, progress, transactions — without the staff account that granted coins), `messagesSent` (only what the person wrote; other people's messages are their data), `conversations` (id, title, group or not, when joined) and `teamApplications`.

## §41 — The popunder is off: it swallowed the first click

**Symptom:** on the home page a visitor clicked a work, the page seemed to load slowly, and they ended up back on the home page. **Cause, reproduced in the browser:** the ad network's popunder script puts an invisible link over the whole page (`document.elementFromPoint` at a work's card returned the network's `<a>`, pointing at an advertiser's domain), so the first click never reaches the site's own link — it opens the advertiser (in a new tab, or by leaving the page) and the visitor is still on the page they were on. It also explains the tab title lure ("(1) New Message!") and the redirect to an unrelated domain seen earlier.

**Now:** `POPUNDER_ENABLED` (`lib/ads.ts`) is false unless `NEXT_PUBLIC_ADS_POPUNDER=true` is set at build time, so the click trap is not loaded; the social bar, the banners and the labelled sponsored links stay. With it off, the first click on a card navigates at once (checked). The remaining ad scripts are also loaded only when the browser is idle (`requestIdleCallback`, at most 5 s), so they no longer compete with loading and hydration, and leaving the page first cancels the load. Turning the popunder back on is a one-variable change; it trades smoothness for the network's highest-paying format, which is the owner's call.

**Still heavy (not changed):** every page's HTML carries the whole catalogue for the client-side provider (about 190 kB for a work's page and 400 kB for the home page on the live site), which costs time on a phone. Sending only what a page needs is the next lever.

## §42 — Requests to open a team are on the server

**Before:** `/teams/create` saved the request in the requester's own browser and `/admin/team-requests` read the reviewer's own browser, so a request never reached anyone and a manager could not see one. **Now** (`modules/team-requests`, table `team_requests`, migration `20260930220000_add_team_requests`, routes under `/v1/team-requests`, all needing an account):
- `POST /` sends a request (name, description, goals, category, expected size, wanted roles, links, colour). At most **two waiting** at once, **three a day**, and never a name a team already has (case-insensitive); a banned account is refused. Every owner, super administrator and global team manager gets a notification linking to `/admin/team-requests`.
- `GET /me` lists a member's own; `GET /?status=` lists all (team managers only — moderators are not); `PUT /:id` lets the requester resend a request that was **sent back for changes** (it returns to waiting; the link in the notification is `/teams/create?edit=<id>`).
- `PATCH /:id/review` (team managers): `pending`/`needs_modification` → `approved`, `rejected` or `needs_modification`; `approved` → `suspended` or `archived`; `suspended` → `approved` (reactivates the *same* team) or `archived`; `rejected` and `archived` are final. **Approving creates the team**: the requester is its leader, the wanted roles become its open positions (so it shows as recruiting, §38), it starts `active` with the chosen colour; if the name was taken in the meantime the approval is refused. Suspending or archiving an approved request sets the same status on its team. Each decision is audited (`team_request.<status>`), drops the catalogue cache and notifies the requester with the reviewer's note.
- The suggested logo link is shown to the reviewer only — the site never fetches it — so an approved team starts with a coloured-letter logo. The requests belong to the requester: they are in the data export (`teamCreationRequests`) and go with the account.

Still kept per browser: the team dashboard's departments, custom roles, collaboration requests and activity log (member roles and removals moved to the server, §43).

## §43 — Promoting, demoting and removing members from the team dashboard is real, and ranked

The dashboard's promote, demote and remove buttons used to change a copy in the leader's browser, so the member never actually moved. For a team on the site they now call `PUT` and `DELETE /v1/catalog/teams/:id/members/:userId` (already there for *Admin › Teams*); a team that exists only in a browser keeps the old local behaviour. The dashboard ignores the browser's copy of a real team's roster and roles, so a stale local change can no longer hide what the site holds.

Those routes were open to anyone holding a team role that counts as "lead" (`team_administrator` up), which let an administrator make themselves assistant leader — or hand out the leader role — by calling the API directly. They now follow a **rank** (`teamRoleRank`: trainee 0, member 1, production roles 2, administrator 3, assistant leader 4, leader 5; the site's team managers stand above all): to change someone's role, add them or remove them, the actor must rank above the person **and** above the role being given, and nobody changes their own place. So the leader can make an assistant but not another leader, an assistant can make administrators but not assistants or leaders, and an administrator can change only members and production roles. A refusal is `403 outranked`. The plain `member` and `trainee` roles are accepted so a member can be stepped down. Checked against Postgres (12 cases) as well as in unit tests.

## §44 — Deleting one's own messages, and blocking

**Deleting.** `DELETE /v1/conversations/:id/messages/:messageId` removes a message the caller wrote (a member of the conversation only; another person's message is `403`, one from another conversation `404`). It is a real delete — the row is gone for everyone, and an open chat drops it within a few seconds because the polling page treats the newest page as authoritative for its own time window.

**Blocking** (table `user_blocks`, migration `20260930230000_add_user_blocks`; `GET /v1/conversations/blocks`, `PUT` and `DELETE /v1/conversations/blocks/:userId`). While a block stands, in either direction, the two cannot start a direct chat, write in the one they share, put each other in a group or be added to the other's; the answer is the same `403 cannot_message` whoever made the block, so a blocked person is not told who blocked them (and their own list does not show it). Old messages stay readable, and a group is not checked when writing (the way out of a group is to leave it). Only the blocker sees the block: the chat page shows a "you blocked this person" bar in place of the message box, a header button toggles it, and the list of blocked people has its own dialog. Nobody blocks themselves; the list is in the data export (`blockedAccounts`) and goes with the account. Checked against Postgres (23 cases) and in unit tests.
