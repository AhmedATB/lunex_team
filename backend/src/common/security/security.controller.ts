...
export class SecurityController {
   ...
   * `keyConfigured` / `keyPresented` say which side is missing it — never the key itself.
   */
  @Public()
  @Get("client")
  ...
  client(@Req() req: Request) {
    return {
      ip: req.context.ip,
      verified: req.context.ipVerified,
      keyConfigured: Boolean(this.config.get<string>("BFF_SHARED_KEY")),
      keyPresented: Boolean(req.header("x-lunex-bff-key")),
    };
  }
}
