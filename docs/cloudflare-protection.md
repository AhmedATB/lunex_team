...
4. Behind Cloudflare, the visitor's address arrives in `CF-Connecting-IP`; the app currently reads Railway's forwarded address, so audit logs and per-IP limits would see Cloudflare's servers. Before cutover, teach the frontend to take that header (only when the request really came through Cloudflare, e.g. behind a Transform Rule that adds a secret header). Not done yet — it changes how every request is attributed.
